...
import {
    LayoutDashboard, Server, Cpu, Eye, Boxes, Layers3, Rocket,
    Settings, CircleDot, Activity, Gauge, Container,
} from "lucide-react";
...
const GROUPS = [
    ...
    },
    {
        ...
        items: [
            ...
            { to: "/aggregator", label: "Host Aggregator", icon: Layers3 },
            { to: "/placement", label: "VM Placement", icon: Rocket },
        ],
    },
    {
        label: "System",
...
export default function Sidebar() {
    return (
        <aside data-testid="app-sidebar" className="hidden md:flex flex-col w-[240px] shrink-0 border-r border-border bg-sidebar h-screen sticky top-0">
                        ...
                        {group.items.map(({ to, label, icon: Icon, soon }) => (
                            <NavLink
                                ...
                                className={({ isActive }) => [
                                    ...
                                    isActive ? "bg-accent-soft text-foreground border-l-2 border-accent pl-[10px]" : "text-muted-foreground",
                                ].join(" ")}
                            >
                                <Icon className="w-4 h-4" />
                                <span className="flex-1">{label}</span>
                                {soon && (
                                    <span className="text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded bg-card-elev text-muted-foreground border border-border">Soon</span>
                                )}
                            </NavLink>
                        ))}
                    </div>
